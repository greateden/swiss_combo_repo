...
def main() -> None:
    repo_id = os.environ.get("SWISS_TO_STANDARD_MODEL_ID", "google/madlad400-3b-mt")
    local_dir = Path(os.environ["SWISS_TO_STANDARD_MODEL_DIR"]).resolve()
    ...
    if not any(path.endswith(".safetensors") or path == "pytorch_model.bin" for path in files):
        raise RuntimeError(f"{repo_id} does not contain supported PyTorch/safetensors weights")
    ...
    print(f"Swiss-to-Standard model ready at {local_dir}")
